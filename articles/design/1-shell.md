...
- A chat panel UI based on
  [`shinychat::chat_mod_ui()`](https://rdrr.io/pkg/shinychat/man/chat_mod_ui.html).
- An extension server that reads `llm_model` from the board options,
  ...
  [`shinychat::chat_mod_server()`](https://rdrr.io/pkg/shinychat/man/chat_mod_ui.html).
...
type-strict check in
[`ellmer::contents_replay()`](https://ellmer.tidyverse.org/reference/contents_record.html)
rejects, and typed props come back mistyped — `tokens` as a list, and
the `"NA"` strings seen below failing S7 validation. Because the replay
runs in a board-server observer, a board saved after any chat turn
aborted the whole board on restore.

The record/replay pair is still what turns a `Turn` into data, but the
result is now stored as a single opaque
[`jsonlite::serializeJSON()`](https://jeroen.r-universe.dev/jsonlite/reference/serializeJSON.html)
string, which preserves R’s types exactly and so round-trips through the
board file unchanged. The saved window is bounded by the
`blockr.chat_save_turns` option, read at save time, the raw provider
response in `json` is stripped, and truncation is trimmed to whole
exchanges so a window never opens on a tool result whose request fell
outside it. The structure below is therefore still the payload — it is
simply no longer the thing written to disk.
