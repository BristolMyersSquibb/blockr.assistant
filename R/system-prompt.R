...
default_system_prompt <- function(board = NULL, client = NULL,
  ...
  intro <- paste(
    ...
    "",
    "Views and layout: a board can have multiple named views (tabs).",
    "Each view holds an arrangement of block + extension panels.",
    "Use get_views / get_layout to inspect, add_view to create a new",
    "view (switches to it on creation), rename_view / remove_view /",
    "set_active_view as expected, and set_layout to arrange the",
    "ACTIVE view from a JSON nested-array grid. View/layout calls",
    "apply immediately (they are NOT staged with block/link",
    "changes). For a multi-branch dashboard, the natural pattern is:",
    "(1) add blocks + links for all branches, (2) add_view per",
    "branch scoped via blocks_json, (3) set_layout on each view to",
    "arrange that branch's panels (switching active view between",
    "calls via set_active_view).",
    "",
    "Answer concisely.",
    sep = "\n"
  )
