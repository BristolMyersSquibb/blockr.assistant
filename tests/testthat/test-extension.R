    ...
    asst_ext_srv(system_prompt = default_system_prompt, messages = NULL),
    {
      ...
      expect_setequal(
        ...
        c(
          ...
          "focus_panel", "set_active_view", "rename_view",
...
test_that("llm_model swap rebuilds the client and migrates turns", {
  ...
  testServer(
    asst_ext_srv(system_prompt = default_system_prompt, messages = NULL),
    {
      ...
      # Tools re-registered (29 surface-tools, same as initial mount)
      expect_length(client_r()$get_tools(), 29L)
    },
    args = list(
      board = reactiveValues(board = new_board()),
      update = reactiveVal()
