...
>
> **Assistant**: \$\$calls \`inspect_results(code =
...
off-screen graphics device open, so a
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) call, an
auto-printed ggplot or lattice object, or an auto-printed plot recording
all return the rendering alongside the text output – one image per page
drawn, sized by the `width` and `height` the model asks for.

Only base R is attached, so the model prefixes calls from other packages
([`graphics::hist()`](https://rdrr.io/r/graphics/hist.html),
`ggplot2::ggplot()`) – the same thing it would write inside a code
block. A plot block’s result is a list of recordings, drawn with
[`evaluate::replay()`](https://bristolmyerssquibb.github.io/blockr.assistant/articles/evaluate.r-lib.org/reference/replay.md),
which is the call the board renders it with.
