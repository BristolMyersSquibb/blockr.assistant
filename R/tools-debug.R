...
tool_run_block_script <- function(board, update, session) {
  ...
  ellmer::tool(
    ...
    description = paste(
      "Run a block's code OFF the board and find out which statement fails.",
      ...
      "usually `data`) and names the statement that raised, with its text.",
      "Use it the moment a block reports an error, BEFORE editing and",
      "committing again: a commit tells you only that the block still",
      "fails, this tells you where.",
      "Pass `code` to try a candidate fix against the same inputs without",
      "touching the board; leave it out to run what the block runs now.",
