...
# default_system_prompt() golden on a populated board
    ...
    Output
      ...
      - `remove_block(id)`: Remove a block from the board. Any links to or from the block are cleaned up for you, both the ones already on the board and the ones you staged this turn; the model does not need to remove them explicitly.
