...
The package exports five S3 generics that let block / stack
authors customise how their classes appear to the model:
...
The two pairs (`summarise_*` vs `describe_*`) split density from
detail: `summarise_block` runs on every turn and should stay
compact (one line); `describe_block` runs only when the model
calls the tool and can be as detailed as the model needs.
