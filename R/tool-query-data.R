tool_query_data <- function(board, update, session) {
  ...
  ellmer::tool(
    ...
    description = paste(
      ...
      "individual tables). Reach a table via `<id>$<table>` or",
      "`dm::dm_get_tables(<id>)` -- e.g. if the dm block id is `dm`,",
      "write `head(dm$adsl)` or `names(dm$adae)`, not bare `adsl`. To",
      "actually use a dm table in a chart/summary, add a dm_pull_block."
    ),
    arguments = list(
