...
`p<n>-<topic>.qmd` (the `p` prefix keeps the file names portable — R
CMD check rejects vignette names starting with a digit). Phases are
ordered so that the package compiles and demos something at the end of
each one.
...
- A visible "stop" affordance backed by `ellmer::stream_controller()`
...
- **Conversation persistence** — bookmarking conversation history across
  sessions; partially supported by `shinychat::chat_mod_server(bookmark_on_*)`.
